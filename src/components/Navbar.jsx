...
function Navbar() {
  ...
  return (
    <div className='absolute w-full flex justify-between items-center p-4'>
        <h1 className='text-3xl font-bold text-white z-20'>Experiences</h1> 
        {nav ? <FaTimes onClick={handleNav} className='z-20 cursor-pointer' size={25} style={{color:'#fff'}} />  
 : <FaBars onClick={handleNav} className='z-20 cursor-pointer' size={25} style={{color:'#fff'}} />  
}
        <div className={nav ? 'fixed text-gray-300 left-0 top-0 w-full h-full bg-black/90 px-4 py-7 ease-linear duration-500 z-10' : 'absolute top-0 left-[-100%] h-full ease-out duration-600 z-10' }>
